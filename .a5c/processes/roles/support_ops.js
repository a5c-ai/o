...
export const supportBacklogQueueWorkerForever = async ({
  pollBatch,
  handleOne,
  ...
  if (typeof pollBatch !== "function") {
    throw new Error("supportBacklogQueueWorkerForever: pollBatch must be a function");
  }
  if (typeof handleOne !== "function") {
    throw new Error("supportBacklogQueueWorkerForever: handleOne must be a function");
  }
  ...
  for (;;) {
    ...
    try {
      ...
      logger?.error?.("[support_ops] poll error", err);
      items = [];
    }

    if (!Array.isArray(items) || items.length === 0) {
      logger?.info?.(`[support_ops] backlog empty; sleeping ${emptySleepMs}ms`);
    ...
    for (const item of items) {
      try {
        ...
        logger?.error?.("[support_ops] handle error", err);
      }
